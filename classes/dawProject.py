...
class DawProject:
    ...
    def export_schema(file, cls):
        try:
            schema_root = etree.Element("xs:schema", nsmap={'xs': "http://www.w3.org/2001/XMLSchema"})
            schema_tree = etree.ElementTree(schema_root)
            with open(file, 'wb') as schema_file:
                schema_tree.write(schema_file, pretty_print=True, xml_declaration=True, encoding="UTF-8")
    ...
    def to_xml(obj):
        try:
            ...
            for key, value in vars(obj).items():
                if isinstance(value, list):  # Handling lists, such as scenes or structure
                    list_elem = ET.SubElement(root, key.capitalize())
                    for item in value:
                        list_elem.append(DawProject.to_xml_element(item))
                elif hasattr(value, 'to_xml'):  # Handle nested objects with a to_xml method
    ...
    def save(project, metadata, embedded_files, file):
        ...
        with ZipFile(file, 'w') as zos:
            DawProject.add_to_zip(zos, DawProject.METADATA_FILE, metadata_xml.encode('utf-8'))
            DawProject.add_to_zip(zos, DawProject.PROJECT_FILE, project_xml.encode('utf-8'))
