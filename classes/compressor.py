...
class Compressor(BuiltInDevice):
    def __init__(self, device_name=None, device_role=None, threshold=None, ratio=None, attack=None, release=None, input_gain=None, output_gain=None, auto_makeup=None):
        super().__init__()
        ...
        self.threshold = threshold if isinstance(threshold, RealParameter) else RealParameter(threshold)
        self.ratio = ratio if isinstance(ratio, RealParameter) else RealParameter(ratio)
        self.attack = attack if isinstance(attack, RealParameter) else RealParameter(attack)
        self.release = release if isinstance(release, RealParameter) else RealParameter(release)
        ...
        self.auto_makeup = auto_makeup if isinstance(auto_makeup, BoolParameter) else BoolParameter(auto_makeup)
    ...
    def to_xml(self):
        ...
        add_real_parameter_elem(compressor_elem, "Attack", self.attack, Unit.SECONDS.value)
        ...
        add_real_parameter_elem(compressor_elem, "InputGain", self.input_gain, Unit.DECIBEL.value)
        add_real_parameter_elem(compressor_elem, "OutputGain", self.output_gain, Unit.DECIBEL.value)
        ...
        add_real_parameter_elem(compressor_elem, "Release", self.release, Unit.SECONDS.value)

        add_real_parameter_elem(compressor_elem, "Threshold", self.threshold, Unit.DECIBEL.value)
    ...
    def from_xml(cls, element):
        ...
        return cls(device_name=device_name, device_role=device_role, threshold=threshold, ratio=ratio, attack=attack, release=release, input_gain=input_gain, output_gain=output_gain, auto_makeup=auto_makeup)