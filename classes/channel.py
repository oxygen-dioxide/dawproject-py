...
class Channel(Lane):
    def __init__(self, role=None, audio_channels=2, volume=None, pan=None, mute=None, solo=None, destination=None,
                 sends=None, devices=None, name=None, color=None, comment=None):
    ...
    def to_xml(self):
        ...
        if self.audio_channels is not None:
            channel_elem.set("audioChannels", str(self.audio_channels))  # Convert int to string
        if self.solo is not None:
            channel_elem.set("solo", str(self.solo).lower())  # Convert boolean to lowercase string
        if self.destination:
            channel_elem.set("destination", str(self.destination.id))  # Assuming destination has an id attribute
        ...
        if self.pan:
            ...
            pan_elem.set("value", str(self.pan.value) if self.pan.value is not None else "")
            pan_elem.set("unit", str(self.pan.unit.value) if self.pan.unit is not None else "")
        if self.sends:
        ...
        if self.volume:
            ...
            volume_elem.set("value", str(self.volume.value) if self.volume.value is not None else "")
            volume_elem.set("unit", str(self.volume.unit.value) if self.volume.unit is not None else "")
    ...
    def from_xml(cls, element):
        ...
        instance.audio_channels = int(audio_channels) if audio_channels is not None else 2
        ...
        instance.mute = BoolParameter.from_xml(mute_elem) if mute_elem is not None else None
