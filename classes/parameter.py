...
class Parameter(Referenceable):
    ...
    def to_xml(self):
        parameter_elem = super().to_xml()  # Call the superclass to_xml to get common attributes
        if self.parameter_id is not None:
            parameter_elem.set("parameterID", str(self.parameter_id))  # Set the parameterID attribute
